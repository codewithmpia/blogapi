...
class PostSerializer(ma.SQLAlchemyAutoSchema):
    ...
    image_url = ma.String(dump_only=True)  # Ajouter un champ pour le lien vers l'image

    class Meta:
        model = Post
        include_relationships = True
        fields = ("title", "slug", "author", "resume", "content", "image_url", "views", "created_at", "publish", "comments")
    ...
    def add_image_url(self, data, **kwargs):
        if isinstance(data, Post):
            data.image_url = url_for('static', filename=data.image, _external=True)  # Utiliser url_for pour générer le lien vers l'image
        return data
